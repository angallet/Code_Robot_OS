...
void parseMessage (int sendingTeam, const unsigned char *buf, int nbbytes) {
  ...
  if (buf[3] != 0xFF && (buf[3] == 0 || buf[3] > game.nbTeams || !game.teams[sendingTeam].active)) {
...
*/
      game.teams[sendingTeam].score += scoreValue;
      time ( &rawtime );
      timeinfo = localtime ( &rawtime );
      log (KNRM, alinea);
      log (KNRM, "  Team %s scored %d point(s) at time %s. Total SCORE: %d points\n", game.teams[sendingTeam].name, scoreValue, asctime (timeinfo), game.teams[sendingTeam].score);
	  (*GUI.notify) ();
      break;
    }
  ...
  case MSG_ACK:
	...
	break;
      ...
      log (COL(1), "%s\n", game.teams[sendingTeam].name);
      log (KNRM, alinea);
      log (KNRM, "          ACK      idAck=%d status=%d\n", idAck, buf[7]);

      if (buf[3] != 0xFF && !game.teams[sendingTeam].connected) {
	log (KRED, "*** Team ");
	log (COL(1), "%s", game.teams[sendingTeam].name);
	log (KRED, " is not connected. Message discarded ***\n");
	break;
      }

      if (buf[3] != 0xFF) {
	write_to_client (&game.teams[sendingTeam], (char *) buf, 8);
      }
  ...
  case MSG_CUSTOM:
	...
	if ((i-5) % 4 == 3)
      ...
      if (!game.teams[sendingTeam].connected) {
	log (KRED, "*** Team ");
	log (COL(1), "%s", game.teams[sendingTeam].name);
	log (KRED, " is not connected. Message discarded ***\n");
	break;
...
int main (int argc, char **argv) {
  ...
  for (team=0; team<15; team++){
    ...
    time ( &rawtime );
    timeinfo = localtime ( &rawtime );
    log (KRED, "Contest started at time = %s\n", asctime (timeinfo));
    /*log (KNRM, "     Test String\n");
    log (KNRM, "     Operating System: %s\n", getOsName());
    log (KBLU, "     Team SolidSnakeIta scores 1!\n");
    log (KGRN, "     Team SolidSnakeIta scores 3!\n");*/ 
